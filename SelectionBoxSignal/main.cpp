...
bool loadLayer(GisViewer& viewer, const QString& path, const QString& displayName, const GisLayerStyle& style)
{
    ...
    if (!viewer.addLayerFromPath(path, &errorMessage))
    {
        QMessageBox::critical(
            nullptr,
            QStringLiteral("SelectionBoxSignal"),
    ...
    int hitCount)
{
    ...
    table.setItem(row, 1, new QTableWidgetItem(rectText(screenRect)));
    table.setItem(row, 2, new QTableWidgetItem(extentText(worldExtent)));
    table.setItem(row, 3, new QTableWidgetItem(modifiersText(modifiers)));
    table.setItem(row, 4, new QTableWidgetItem(QString::number(hitCount)));
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("SelectionBoxSignal"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("SelectionBoxSignal"));
    ...
    QAction* selectAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/select.svg")), QStringLiteral("Box Select"));
    selectAction->setCheckable(true);
    selectAction->setChecked(true);
    QAction* panAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/pan.svg")), QStringLiteral("Pan"));
    panAction->setCheckable(true);
    QAction* clearSelectionAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/delete.svg")), QStringLiteral("Clear Selection"));
    QAction* fullExtentAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/full-extent.svg")), QStringLiteral("Full Extent"));

    auto* stateLabel = new QLabel(QStringLiteral("Signal: mapSelectionBoxFinished(rect, extent, modifiers)"), &window);
    ...
    signalTable->setColumnCount(5);
    signalTable->setHorizontalHeaderLabels({
        QStringLiteral("Time"),
        QStringLiteral("Screen rect"),
        QStringLiteral("World extent"),
    ...
    auto* signalDock = new QDockWidget(QStringLiteral("mapSelectionBoxFinished log"), &window);
    ...
    QObject::connect(selectAction, &QAction::triggered, viewer, [&](bool checked) {
        if (checked)
        {
            ...
            viewer->setActiveTool(GisViewerTool::Select);
            window.statusBar()->showMessage(QStringLiteral("Drag a box to log mapSelectionBoxFinished parameters."));
        }
    ...
    QObject::connect(viewer, &GisViewer::mapSelectionBoxFinished, viewer, [&](
        ...
        appendSignalLog(*signalTable, screenRect, worldExtent, modifiers, hits.size());
        ...
        window.statusBar()->showMessage(QStringLiteral("mapSelectionBoxFinished: rect=%1 extent=%2 modifiers=%3 hits=%4")
            .arg(rectText(screenRect))
            .arg(extentText(worldExtent))
