...
inline bool downloadFile(const QUrl& url, const QString& targetPath, QWidget* parent)
{
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("GeoKernel LayerVisibility"));
    ...
    if (!succeeded)
    {
        ...
        QMessageBox::critical(parent, QStringLiteral("LayerVisibility"),
    ...
    QWidget* parent)
{
    ...
    const QString zipPath = QDir(dataRoot).absoluteFilePath(archiveName);
    if (!QFileInfo::exists(zipPath) &&
        !downloadFile(url, zipPath, parent))
        return {};
