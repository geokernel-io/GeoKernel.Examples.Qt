...
{
    return polygonFromRing({
        GisShapePoint(-0.2, 3.0),
        GisShapePoint(0.6, 1.2),
        GisShapePoint(3.2, 1.2),
        GisShapePoint(1.1, -0.1),
        GisShapePoint(2.0, -2.0),
        GisShapePoint(-0.2, -0.8),
        GisShapePoint(-2.4, -2.0),
        GisShapePoint(-1.5, -0.1),
        GisShapePoint(-3.6, 1.2),
        GisShapePoint(-1.0, 1.2),
        GisShapePoint(-0.2, 3.0)
    });
}
    ...
    bool showResult)
{
    ...
    QStringList details;
    details << QStringLiteral("Difference(left, right)");
    details << QStringLiteral("This calculates left - right.");
    ...
    if (!showResult)
    {
        details << QStringLiteral("Result: click Run Difference to calculate");
        statusBar.showMessage(QStringLiteral("Source polygons are ready. Click Run Difference."));
    }
    else
    {
        GisTopology topology;
        std::unique_ptr<GisShape> result = topology.Difference(left, right);
        if (result && !result->isEmpty())
        {
            ...
            statusBar.showMessage(QStringLiteral("Difference result created."));
        }
        else
        {
            details << QStringLiteral("Result: empty");
            statusBar.showMessage(QStringLiteral("Difference returned an empty result."));
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("Difference"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(980, 680);
    window.setWindowTitle(QStringLiteral("Difference"));
    ...
    toolbar->addWidget(new QLabel(QStringLiteral("Operation: Difference(left, right)"), toolbar));
    toolbar->addSeparator();
    ...
    QObject::connect(runDifferenceAction, &QAction::triggered, &window, [viewer, detailsDock, &window]
    {
