...
bool loadLayer(GisViewer& viewer, const QString& path)
{
    ...
    QMessageBox::critical(
        nullptr,
        QStringLiteral("MoveFeatureProgrammatic"),
...
QString selectionText(const GisViewer& viewer)
{
    ...
    lines << QStringLiteral("Use moveSelectedFeaturesInEditLayer(deltaX, deltaY) buttons to move selection.");
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("MoveFeatureProgrammatic"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("MoveFeatureProgrammatic"));
    ...
    auto* deltaLabel = new QLabel(QStringLiteral("Delta:"), &window);
    deltaLabel->setContentsMargins(8, 0, 4, 0);
    toolbar->addWidget(deltaLabel);

    auto* deltaSpin = new QDoubleSpinBox(&window);
    deltaSpin->setDecimals(2);
    deltaSpin->setRange(0.10, 30.0);
    deltaSpin->setSingleStep(0.50);
    deltaSpin->setValue(3.00);
    deltaSpin->setSuffix(QStringLiteral(" deg"));
    deltaSpin->setMaximumWidth(110);
    toolbar->addWidget(deltaSpin);

    QAction* moveWestAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/move.svg")), QStringLiteral("Move West"));
    QAction* moveEastAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/move.svg")), QStringLiteral("Move East"));
    QAction* moveNorthAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/layer-up.svg")), QStringLiteral("Move North"));
    QAction* moveSouthAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/layer-down.svg")), QStringLiteral("Move South"));
    toolbar->addSeparator();
    QAction* resetAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/refresh.svg")), QStringLiteral("Reset Points"));
    ...
    selectionInfo->setPlainText(QStringLiteral("Select mode: click a point. Ctrl+click toggles multiple selection. Move buttons call moveSelectedFeaturesInEditLayer(deltaX, deltaY)."));
    ...
    const QString worldPath = ensureSampleFile(
        ...
        &window);
    if (worldPath.isEmpty() || !loadLayer(*viewer, worldPath))
        return 1;
    ...
    const auto moveSelection = [&](double deltaX, double deltaY) {
        if (viewer->selectedFeatureCount() <= 0)
        {
            window.statusBar()->showMessage(QStringLiteral("Select one or more points first."));
            return;
        }

        if (!viewer->moveSelectedFeaturesInEditLayer(deltaX, deltaY))
        {
            window.statusBar()->showMessage(QStringLiteral("moveSelectedFeaturesInEditLayer failed."));
            return;
        }

        refreshViewer(*viewer);
        selectionInfo->setPlainText(QStringLiteral("moveSelectedFeaturesInEditLayer(%1, %2)\n\n%3")
            .arg(deltaX, 0, 'f', 2)
            .arg(deltaY, 0, 'f', 2)
            .arg(selectionText(*viewer)));
        window.statusBar()->showMessage(QStringLiteral("Moved selected feature(s) by deltaX=%1 deltaY=%2.")
            .arg(deltaX, 0, 'f', 2)
            .arg(deltaY, 0, 'f', 2));
    };

    QObject::connect(moveWestAction, &QAction::triggered, viewer, [&, deltaSpin] {
        moveSelection(-deltaSpin->value(), 0.0);
    });
    QObject::connect(moveEastAction, &QAction::triggered, viewer, [&, deltaSpin] {
        moveSelection(deltaSpin->value(), 0.0);
    });
    QObject::connect(moveNorthAction, &QAction::triggered, viewer, [&, deltaSpin] {
        moveSelection(0.0, deltaSpin->value());
    });
    QObject::connect(moveSouthAction, &QAction::triggered, viewer, [&, deltaSpin] {
        moveSelection(0.0, -deltaSpin->value());
    });
