...
#include "Vector/Mif/GisLayerMIF.h"
#include "Viewer/GisViewer.h"

using namespace GeoKernel::Core::Layers;
using namespace GeoKernel::Core::Layers::Defs;
using namespace GeoKernel::Formats::Vector::Mif;
...
QString detailsText(const QString& path, const GisLayerMIF& layer)
{
    QStringList lines;
    lines << QStringLiteral("MifLoad sample");
    lines << QStringLiteral("");
    lines << QStringLiteral("API");
    lines << QStringLiteral("GisLayerMIF(path)");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("Loaded MapInfo MIF");
    ...
    lines << QStringLiteral("Provider-backed MIF files can render features while memory shape count remains 0.");
    ...
    lines << sidecarLine(QStringLiteral(".mif"), path);
    lines << sidecarLine(QStringLiteral(".mid"), layer.midPath());
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("MifLoad"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 760);
    window.setWindowTitle(QStringLiteral("MifLoad"));
    ...
    const QString path = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/albania.zip")),
        QStringLiteral("albania.zip"),
        QStringLiteral("albania_mif"),
        QStringLiteral("albania.mif"),
        &window);
    if (path.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("MapInfo MIF sample data could not be prepared."));
        return 1;
    }
    auto layer = std::make_unique<GisLayerMIF>(path);
    layer->style() = mifStyle();
    ...
    catch (const std::exception& ex)
    {
        QMessageBox::critical(
            &window,
            QStringLiteral("MifLoad"),
            QStringLiteral("MapInfo MIF could not be opened:\n%1").arg(QString::fromUtf8(ex.what())));
        return 1;
    }

    GisLayerMIF* layerPtr = layer.get();
    ...
    window.statusBar()->showMessage(QStringLiteral("GisLayerMIF opened %1 memory features and %2 fields.")
        .arg(layerPtr->count())
