...
void showEmptyInfo(QTableWidget& table)
{
    table.setRowCount(1);
    setRow(table, 0, QStringLiteral("mapClicked"), QStringLiteral("Click the map while GisViewerTool::Info is active."));
}

void showInfo(QTableWidget& table, const QPointF& screenPoint, const GisShapePoint& worldPoint, const FeatureHitTestResult& hit)
{
    ...
    table.setRowCount(8 + keys.size());
    setRow(table, 0, QStringLiteral("Tool"), QStringLiteral("GisViewerTool::Info"));
    setRow(table, 1, QStringLiteral("Signal"), QStringLiteral("mapClicked(tool, screenPoint, worldPoint, modifiers)"));
    setRow(table, 2, QStringLiteral("Screen point"), pointText(screenPoint));
    setRow(table, 3, QStringLiteral("World point"), pointText(worldPoint));
    setRow(table, 4, QStringLiteral("Layer"), hit.layer != nullptr ? hit.layer->name() : QStringLiteral("-"));
    setRow(table, 5, QStringLiteral("Feature id"), QString::number(hit.featureId));
    setRow(table, 6, QStringLiteral("Shape type"), hit.shape != nullptr ? shapeTypeName(hit.shape->shapeType()) : QStringLiteral("-"));
    setRow(table, 7, QStringLiteral("Extent"), hit.shape != nullptr ? extentText(hit.shape->extent()) : QStringLiteral("-"));
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("InfoTool"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("InfoTool"));
    window.statusBar()->showMessage(QStringLiteral("Info tool is active. Click the map to receive mapClicked."));
    ...
    QAction* identifyAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/select.svg")), QStringLiteral("Info Tool"));
    ...
    auto* stateLabel = new QLabel(QStringLiteral("Tool: GisViewerTool::Info | Signal: mapClicked"), &window);
    ...
    showEmptyInfo(*detailsTable);

    auto* detailsDock = new QDockWidget(QStringLiteral("Info tool click details"), &window);
    ...
    QObject::connect(identifyAction, &QAction::triggered, viewer, [&](bool checked) {
        if (checked)
        {
            ...
            viewer->setActiveTool(GisViewerTool::Info);
            window.statusBar()->showMessage(QStringLiteral("GisViewerTool::Info active. Click the map."));
        }
    ...
    QObject::connect(viewer, &GisViewer::mapClicked, viewer, [&](
        ...
        if (!hit.isValid())
        {
            ...
            setRow(*detailsTable, 0, QStringLiteral("Tool"), QStringLiteral("GisViewerTool::Info"));
            setRow(*detailsTable, 1, QStringLiteral("Signal"), QStringLiteral("mapClicked"));
            setRow(*detailsTable, 2, QStringLiteral("Screen point"), pointText(screenPoint));
            setRow(*detailsTable, 3, QStringLiteral("World point"), pointText(worldPoint));
            window.statusBar()->showMessage(QStringLiteral("mapClicked received, no feature hit."));
            return;
        }

        Q_UNUSED(modifiers);
        viewer->setSelectedFeature(hit);
        showInfo(*detailsTable, screenPoint, worldPoint, hit);
        window.statusBar()->showMessage(QStringLiteral("mapClicked with GisViewerTool::Info: %1 feature %2")
