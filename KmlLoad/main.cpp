...
#include "Vector/Kml/GisLayerKML.h"
#include "Viewer/GisViewer.h"

using namespace GeoKernel::Core::Layers;
using namespace GeoKernel::Core::Layers::Defs;
using namespace GeoKernel::Formats::Vector::Kml;
using namespace GeoKernel::Viewer;

QString ensureKmlDocument(const QString& kmzPath, QWidget* parent)
{
    const QString targetDir = QDir(QFileInfo(kmzPath).absolutePath())
        .absoluteFilePath(QStringLiteral("travel_kml_content"));
    const QString existing = findFile(targetDir, QStringLiteral("doc.kml"));
    if (!existing.isEmpty())
        return existing;
    ...
    const QStringList extractorArguments = {
        QStringLiteral("-xf"), QDir::toNativeSeparators(kmzPath)
    };
#else
    const QString extractorProgram = QStandardPaths::findExecutable(QStringLiteral("unzip"));
    const QStringList extractorArguments = {
        QStringLiteral("-o"), kmzPath, QStringLiteral("-d"), targetDir
    };
#endif

    if (extractorProgram.isEmpty())
    {
        QMessageBox::critical(parent, QStringLiteral("KmlLoad"),
            QStringLiteral("A ZIP extractor is required to open the KMZ sample."));
        return {};
    }
        ...
        || extractor.exitCode() != 0)
    {
        ...
        QMessageBox::critical(parent, QStringLiteral("KmlLoad"),
            details.isEmpty()
                ? QStringLiteral("The KML document could not be extracted from the KMZ sample.")
                : QStringLiteral("The KML document could not be extracted:\n%1").arg(details));
        return {};
    }

    return findFile(targetDir, QStringLiteral("doc.kml"));
}
...
QString fileLine(const QString& label, const QString& path)
{
...
QString detailsText(const QString& path, const GisLayerKML& layer)
{
    QStringList lines;
    lines << QStringLiteral("KmlLoad sample");
    lines << QStringLiteral("");
    lines << QStringLiteral("API");
    lines << QStringLiteral("GisLayerKML(path)");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("Loaded KML/KMZ");
    ...
    lines << QStringLiteral("Provider-backed KML/KMZ files can render features while memory shape count remains 0.");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("File");
    lines << fileLine(QStringLiteral(".kml"), path);
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("KmlLoad"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 760);
    window.setWindowTitle(QStringLiteral("KmlLoad"));
    ...
    const QString kmzPath = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/travel.zip")),
        QStringLiteral("travel.zip"),
        QStringLiteral("travel_kml"),
        QStringLiteral("travel.kmz"),
        &window);
    if (kmzPath.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("KML/KMZ sample data could not be prepared."));
        return 1;
    }

    const QString path = ensureKmlDocument(kmzPath, &window);
    if (path.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("The KML document could not be prepared."));
        return 1;
    }
    auto layer = std::make_unique<GisLayerKML>(path);
    layer->style() = kmlStyle();
    ...
    catch (const std::exception& ex)
    {
        QMessageBox::critical(
            &window,
            QStringLiteral("KmlLoad"),
            QStringLiteral("KML/KMZ could not be opened:\n%1").arg(QString::fromUtf8(ex.what())));
        return 1;
    }

    GisLayerKML* layerPtr = layer.get();
    ...
    window.statusBar()->showMessage(QStringLiteral("GisLayerKML opened %1 memory features and %2 fields.")
        .arg(layerPtr->count())
