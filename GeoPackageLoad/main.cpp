...
#include "Vector/GeoPackage/GisLayerGPKG.h"
#include "Viewer/GisViewer.h"

using namespace GeoKernel::Core::Layers;
using namespace GeoKernel::Core::Layers::Defs;
using namespace GeoKernel::Formats::Vector::GeoPackage;
...
QString detailsText(const QString& path, const GisLayerGPKG& layer)
{
    QStringList lines;
    lines << QStringLiteral("GeoPackageLoad sample");
    lines << QStringLiteral("");
    lines << QStringLiteral("API");
    lines << QStringLiteral("GisLayerGPKG(path)");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("Loaded GeoPackage");
    ...
    lines << QStringLiteral("Provider-backed GeoPackage files can render features while memory shape count remains 0.");
    ...
    lines << fileLine(QStringLiteral(".gpkg"), path);
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("GeoPackageLoad"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 760);
    window.setWindowTitle(QStringLiteral("GeoPackageLoad"));
    ...
    const QString path = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/europe_detailed.zip")),
        QStringLiteral("europe_detailed.zip"),
        QStringLiteral("europe_detailed_gpkg"),
        QStringLiteral("europe_detailed.gpkg"),
        &window);
    if (path.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("GeoPackage sample data could not be prepared."));
        return 1;
    }
    auto layer = std::make_unique<GisLayerGPKG>(path);
    layer->style() = geoPackageStyle();
    ...
    catch (const std::exception& ex)
    {
        QMessageBox::critical(
            &window,
            QStringLiteral("GeoPackageLoad"),
            QStringLiteral("GeoPackage could not be opened:\n%1").arg(QString::fromUtf8(ex.what())));
        return 1;
    }

    GisLayerGPKG* layerPtr = layer.get();
    ...
    window.statusBar()->showMessage(QStringLiteral("GisLayerGPKG opened %1 memory features and %2 fields.")
        .arg(layerPtr->count())
