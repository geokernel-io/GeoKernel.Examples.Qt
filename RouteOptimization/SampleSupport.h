...
inline QString ensureStockholmLayer(QWidget* parent)
{
    const QString dataRoot = sampleDataPath();
    const QString targetDir = QDir(dataRoot).absoluteFilePath(QStringLiteral("stockholm"));
    const QString existing = findFile(targetDir, QStringLiteral("stockholm.shp"));
    if (!existing.isEmpty())
        return existing;

    QDir().mkpath(targetDir);
    const QString zipPath = QDir(dataRoot).absoluteFilePath(QStringLiteral("stockholm.zip"));
    if (!QFileInfo::exists(zipPath) &&
        !downloadFile(QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/stockholm.zip")), zipPath, parent))
        return {};
    ...
    return findFile(targetDir, QStringLiteral("stockholm.shp"));
}
