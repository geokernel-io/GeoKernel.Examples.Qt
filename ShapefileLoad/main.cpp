...
void fillSchemaTable(QTableWidget& table, const GisLayerVector& layer)
{
    ...
    void fillAttributeTable(QTableWidget& table, const GisLayerVector& layer, int maxRows)
    {
        ...
        for (int row = 0; row < rows.size(); ++row)
        {
            ...
            const QVariant value = attributes.value(fieldName);
            table.setItem(row, column + 1, new QTableWidgetItem(value.toString()));
        }
    }
...
QString detailsText(const QString& path, const GisLayerVector& layer)
{
    const QFileInfo shpInfo(path);
    const QFileInfo dbfInfo(shpInfo.absolutePath() + QLatin1Char('/') + shpInfo.completeBaseName() + QStringLiteral(".dbf"));
    const QFileInfo shxInfo(shpInfo.absolutePath() + QLatin1Char('/') + shpInfo.completeBaseName() + QStringLiteral(".shx"));

    QStringList lines;
    lines << QStringLiteral("ShapefileLoad sample");
    lines << QStringLiteral("");
    lines << QStringLiteral("API");
    lines << QStringLiteral("GisLayerSHP(path)");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("Loaded shapefile");
    ...
    lines << QStringLiteral("Shape type: %1").arg(shapeTypeName(layer.shapeType()));
        ...
        lines << QStringLiteral("Provider-backed shapefiles can render features while memory shape count remains 0.");
    ...
    lines << QStringLiteral(".shp: %1 bytes").arg(shpInfo.exists() ? shpInfo.size() : 0);
    lines << QStringLiteral(".shx: %1 bytes").arg(shxInfo.exists() ? shxInfo.size() : 0);
    lines << QStringLiteral(".dbf: %1 bytes").arg(dbfInfo.exists() ? dbfInfo.size() : 0);
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("ShapefileLoad"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 760);
    window.setWindowTitle(QStringLiteral("ShapefileLoad"));
    ...
    QMetaObject::invokeMethod(&window, [&window, viewer, detailsView, schemaTable, attributesTable]
    {
        const QString path = ensureWorldLayer(&window);
        if (path.isEmpty())
        {
            window.statusBar()->showMessage(QStringLiteral("Shapefile sample data could not be prepared."));
            return;
        }

        auto layer = std::make_unique<GisLayerSHP>(path);
        ...
        catch (const std::exception& ex)
        {
            QMessageBox::critical(
                &window,
                QStringLiteral("ShapefileLoad"),
                QStringLiteral("Shapefile could not be opened:\n%1").arg(QString::fromUtf8(ex.what())));
            return;
        }

        GisLayerSHP* layerPtr = layer.get();
        ...
        window.statusBar()->showMessage(QStringLiteral("GisLayerSHP opened %1 features and %2 fields.")
            .arg(layerPtr->count())
