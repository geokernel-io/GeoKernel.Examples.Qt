...
int main(int argc, char* argv[])
{
    ...
    QObject::connect(showLabelsCheck, &QCheckBox::toggled, [&](bool checked) {
    ...
    QObject::connect(fieldCombo, &QComboBox::currentTextChanged, [&](const QString& fieldName) {
        worldLayer->style().setLabelField(fieldName);
        refreshLabels(*viewer);
        window.statusBar()->showMessage(QStringLiteral("Label field: %1").arg(fieldName));
    });

    QObject::connect(fontSizeSpin, qOverload<double>(&QDoubleSpinBox::valueChanged), [&](double value) {
