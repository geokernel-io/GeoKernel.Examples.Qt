...
QString writeShapeWkt(GeometryMode mode, const GisShape* shape)
{
    if (shape == nullptr)
        return QString();
    ...
    switch (mode)
    {
        case GeometryMode::Point:
        {
            ...
            return point != nullptr ? GisWktWriter::writePoint(*point) : QString();
        }
        case GeometryMode::Polyline:
        {
            const auto* polyline = dynamic_cast<const GisShapePolyline*>(shape);
            return polyline != nullptr ? GisWktWriter::writePolyline(*polyline) : QString();
        }
        case GeometryMode::Polygon:
        {
            const auto* polygon = dynamic_cast<const GisShapePolygon*>(shape);
            return polygon != nullptr ? GisWktWriter::writePolygon(*polygon) : QString();
        }
    }
...
QString apiName(GeometryMode mode)
{
    ...
    {
        case GeometryMode::Point:
            return QStringLiteral("GisWktWriter::writePoint(shape)");
        case GeometryMode::Polyline:
            return QStringLiteral("GisWktWriter::writePolyline(shape)");
        case GeometryMode::Polygon:
            return QStringLiteral("GisWktWriter::writePolygon(shape)");
    }
...
QString helpText(GeometryMode mode)
{
    ...
    {
        case GeometryMode::Point:
            return QStringLiteral("Click on the map to draw a point. WKT is written automatically.");
        case GeometryMode::Polyline:
            return QStringLiteral("Click line vertices, then press Enter or double-click to finish. WKT is written automatically.");
        case GeometryMode::Polygon:
            return QStringLiteral("Click polygon vertices, then press Enter or double-click to finish. WKT is written automatically.");
    }
...
int main(int argc, char* argv[])
{
    ...
    window.setWindowTitle(QStringLiteral("WktWrite"));
    ...
    const auto writeWkt = [&]
    {
        ...
        const QString wkt = writeShapeWkt(mode, shape);
        ...
        QStringList details;
        details << QStringLiteral("WktWrite sample");
        ...
        details << QStringLiteral("");
        details << QStringLiteral("Output WKT");
        details << wkt;
        details << QStringLiteral("");
        ...
        details << QStringLiteral("3. WKT is written automatically when drawing finishes.");

        detailsView->setPlainText(details.join(QStringLiteral("\n")));
        window.statusBar()->showMessage(QStringLiteral("%1 wrote WKT from the drawn %2.")
            .arg(apiName(mode))
