...
int main(int argc, char* argv[])
{
    ...
    {
        const QString tiffPath = ensureSampleFile(
        ...
        if (tiffPath.isEmpty())
        {
            window.statusBar()->showMessage(QStringLiteral("GeoTIFF sample data could not be prepared."));
            return;
        }
        ...
        window.statusBar()->showMessage(QStringLiteral("GeoTIFF loaded: world_8km.tif"));
        viewer->fullExtent();
    });
