...
namespace
{
    ...
    QString detailsText(CacheMode mode, const GisLayerGdalRaster* raster, const BenchmarkResult& benchmark, const QString& sourcePath)
    {
        QStringList details;
        details << QStringLiteral("RasterTileCache sample");
        details << QStringLiteral("");
        details << QStringLiteral("Load mode: %1").arg(cacheModeText(mode));
        details << QStringLiteral("Source: %1").arg(sourcePath);
        details << QStringLiteral("");
...
int main(int argc, char* argv[])
{
    ...
    const QString sourceRasterPath = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/world_8km_tif.zip")),
        QStringLiteral("world_8km_tif.zip"),
        QStringLiteral("world_8km_tif"),
        QStringLiteral("world_8km.tif"),
        &window);
    ...
    {
        detailsView->setPlainText(detailsText(currentMode, rasterLayerAt(*viewer), lastBenchmark, sourceRasterPath));
    };
    ...
    const auto loadRaster = [&](CacheMode mode)
    {
        ...
        QString errorMessage;
        const bool loaded = viewer->addLayerFromPath(sourceRasterPath, options, &errorMessage);
        ...
        if (!loaded)
        {
            QMessageBox::critical(
                ...
                QStringLiteral("Raster could not be loaded:\n%1")
                    .arg(errorMessage.isEmpty() ? sourceRasterPath : errorMessage));
