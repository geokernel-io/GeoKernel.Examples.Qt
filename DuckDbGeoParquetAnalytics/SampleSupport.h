...
inline bool downloadFile(const QUrl& url, const QString& targetPath, QWidget* parent)
{
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("GeoKernel DuckDbGeoParquetAnalytics"));
    ...
    if (!succeeded)
    {
        ...
        QMessageBox::critical(parent, QStringLiteral("DuckDbGeoParquetAnalytics"),
...
inline QString ensureStockholmGeoParquet(QWidget* parent)
{
    const QString dataRoot = sampleDataPath();
    const QString targetDir = QDir(dataRoot).absoluteFilePath(QStringLiteral("stockholm_data"));
    const QString existing = findFile(targetDir, QStringLiteral("stockholm_buildings.parquet"));
    if (!existing.isEmpty())
        return existing;

    QDir().mkpath(targetDir);
    const QString zipPath = QDir(dataRoot).absoluteFilePath(QStringLiteral("stockholm_data.zip"));
    if (!QFileInfo::exists(zipPath) &&
        !downloadFile(QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/stockholm_data.zip")), zipPath, parent))
        return {};
    ...
    return findFile(targetDir, QStringLiteral("stockholm_buildings.parquet"));
}
