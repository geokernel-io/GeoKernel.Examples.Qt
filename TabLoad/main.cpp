...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("TabLoad"));
    ...
    const QString path = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/paris_tab.zip")),
        QStringLiteral("paris_tab.zip"),
        QStringLiteral("paris_tab"),
        QStringLiteral("paris.tab"),
        &window);
    if (path.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("MapInfo TAB sample data could not be prepared."));
        return 1;
    }
