...
bool loadLayer(GisViewer& viewer, const QString& path, const QString& displayName, const GisLayerStyle& style)
{
    ...
    if (!viewer.addLayerFromPath(path, &errorMessage))
    {
        QMessageBox::critical(
            nullptr,
            QStringLiteral("ZoomToSelection"),
...
{
    label.setText(QStringLiteral("Selected: %1 | selectedFeaturesExtent: %2")
        .arg(viewer.selectedFeatureCount())
        .arg(extentText(viewer.selectedFeaturesExtent())));
}

int main(int argc, char* argv[])
{
    QApplication app(argc, argv);
    QApplication::setApplicationName(QStringLiteral("ZoomToSelection"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("ZoomToSelection"));
    ...
    QAction* clearAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/delete.svg")), QStringLiteral("Clear Selection"));
    QAction* zoomToSelectionAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/zoom-box.svg")), QStringLiteral("zoomToSelectedFeatures"));
    ...
    QObject::connect(selectAction, &QAction::triggered, viewer, [&](bool checked) {
        if (checked)
        {
            ...
            viewer->setActiveTool(GisViewerTool::Info);
            window.statusBar()->showMessage(QStringLiteral("Click = add to selection, Ctrl+Click = toggle."));
        }
    ...
    QObject::connect(zoomToSelectionAction, &QAction::triggered, viewer, [&] {
        const GisExtent selectedExtent = viewer->selectedFeaturesExtent();
        if (viewer->zoomToSelectedFeatures())
        {
            window.statusBar()->showMessage(QStringLiteral("zoomToSelectedFeatures ok, selectedFeaturesExtent=%1")
                .arg(extentText(selectedExtent)));
        }
        else
        {
            window.statusBar()->showMessage(QStringLiteral("No selected feature extent to zoom."));
        }
    });
