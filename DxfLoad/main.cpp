...
#include "Vector/Dxf/GisLayerDXF.h"
#include "Viewer/GisViewer.h"

using namespace GeoKernel::Core::Layers;
using namespace GeoKernel::Core::Layers::Defs;
using namespace GeoKernel::Formats::Vector::Dxf;
...
GisLayerStyle dxfStyle()
{
    GisLayerStyle style;
    style.setFillColor(QStringLiteral("#D7E5DF"));
    style.setFillOpacity(0.35f);
    style.setLineColor(QStringLiteral("#2E6F91"));
    style.setLineWidth(1.25f);
    style.setPointColor(QStringLiteral("#D95D39"));
    style.setPointSize(6.0f);
    return style;
}
...
QString detailsText(const QString& path, const GisLayerDXF& layer)
{
    QStringList lines;
    lines << QStringLiteral("DxfLoad sample");
    lines << QStringLiteral("");
    lines << QStringLiteral("API");
    lines << QStringLiteral("GisLayerDXF(path)");
    ...
    lines << QStringLiteral("");
    lines << QStringLiteral("Loaded CAD DXF");
    ...
    lines << QStringLiteral("GisLayerDXF parses supported DXF entities into an in-memory vector layer.");
    lines << QStringLiteral("Supported entities include POINT, TEXT, MTEXT, LINE, LWPOLYLINE, POLYLINE, CIRCLE and ARC.");
    ...
    lines << fileLine(QStringLiteral(".dxf"), path);
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("DxfLoad"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 760);
    window.setWindowTitle(QStringLiteral("DxfLoad"));
    ...
    const QString path = ensureSampleFile(
        QUrl(QStringLiteral("https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/geog_25000_dxf.zip")),
        QStringLiteral("geog_25000_dxf.zip"),
        QStringLiteral("geog_25000_dxf"),
        QStringLiteral("geog_25000.dxf"),
        &window);
    if (path.isEmpty())
    {
        window.statusBar()->showMessage(QStringLiteral("DXF sample data could not be prepared."));
        return 1;
    }
    auto layer = std::make_unique<GisLayerDXF>(path);
    layer->style() = dxfStyle();
    ...
    catch (const std::exception& ex)
    {
        QMessageBox::critical(
            &window,
            QStringLiteral("DxfLoad"),
            QStringLiteral("DXF could not be opened:\n%1").arg(QString::fromUtf8(ex.what())));
        return 1;
    }

    GisLayerDXF* layerPtr = layer.get();
    ...
    window.statusBar()->showMessage(QStringLiteral("GisLayerDXF opened %1 features and %2 fields.")
        .arg(layerPtr->count())
