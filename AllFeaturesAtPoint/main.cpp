...
bool loadLayer(GisViewer& viewer, const QString& path, const QString& displayName, const GisLayerStyle& style)
{
    ...
    if (!viewer.addLayerFromPath(path, &errorMessage))
    {
        QMessageBox::critical(
            nullptr,
            QStringLiteral("AllFeaturesAtPoint"),
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("AllFeaturesAtPoint"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("AllFeaturesAtPoint"));
    ...
    QAction* identifyAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/identify.svg")), QStringLiteral("All Features At Point"));
    ...
    auto* stateLabel = new QLabel(QStringLiteral("Tool: hitTestFeaturesAt"), &window);
    ...
    auto* hitsDock = new QDockWidget(QStringLiteral("Features at clicked point"), &window);
    ...
    clearAttributes(*attributesTable, QStringLiteral("Click the map to list intersecting features."));
    ...
    QObject::connect(identifyAction, &QAction::triggered, viewer, [&](bool checked) {
        if (checked)
        {
            ...
            viewer->setActiveTool(GisViewerTool::Info);
            window.statusBar()->showMessage(QStringLiteral("Click a point to list all feature hits."));
        }
    ...
    QObject::connect(viewer, &GisViewer::mapClicked, viewer, [&](
        ...
        currentHits = viewer->hitTestFeaturesAt(screenPoint, 8);
        showHits(*hitsTable, currentHits);

        if (currentHits.isEmpty())
        {
            viewer->clearSelectedFeatures();
            clearAttributes(*attributesTable, QStringLiteral("No feature at clicked point."));
            window.statusBar()->showMessage(QStringLiteral("No feature hit at %1, %2.")
        ...
        window.statusBar()->showMessage(QStringLiteral("%1 feature hit(s) at %2, %3.")
            .arg(currentHits.size())
            .arg(worldPoint.x(), 0, 'f', 6)
            .arg(worldPoint.y(), 0, 'f', 6));
    });
