...
bool loadLayer(GisViewer& viewer, const QString& path, const QString& displayName, const GisLayerStyle& style)
{
    ...
    if (!viewer.addLayerFromPath(path, &errorMessage))
    {
        QMessageBox::critical(
            nullptr,
            QStringLiteral("FeatureAttributes"),
...
void showEmptyAttributes(QTableWidget& table)
{
    table.setRowCount(1);
    setRow(table, 0, QStringLiteral("Attributes"), QStringLiteral("Click a feature to read FeatureHitTestResult::attributes()."));
}
...
void showAttributes(QTableWidget& table, const FeatureHitTestResult& hit)
{
    ...
    setRow(table, 5, QStringLiteral("attributes().count"), QString::number(attributes.size()));
...
int main(int argc, char* argv[])
{
    ...
    QApplication::setApplicationName(QStringLiteral("FeatureAttributes"));
    QApplication::setWindowIcon(QIcon(QStringLiteral(":/icons/geokernel.ico")));

    QMainWindow window;
    window.resize(1200, 800);
    window.setWindowTitle(QStringLiteral("FeatureAttributes"));
    ...
    QAction* identifyAction = toolbar->addAction(QIcon(QStringLiteral(":/icons/select.svg")), QStringLiteral("Feature Attributes"));
    ...
    auto* stateLabel = new QLabel(QStringLiteral("API: FeatureHitTestResult::attributes()"), &window);
    ...
    detailsTable->setHorizontalHeaderLabels({ QStringLiteral("Field"), QStringLiteral("Value") });
    ...
    showEmptyAttributes(*detailsTable);

    auto* detailsDock = new QDockWidget(QStringLiteral("Attributes"), &window);
    ...
    QObject::connect(identifyAction, &QAction::triggered, viewer, [&](bool checked) {
        if (checked)
        {
            ...
            viewer->setActiveTool(GisViewerTool::Info);
            window.statusBar()->showMessage(QStringLiteral("Click a feature to read all attribute values."));
        }
    ...
    QObject::connect(viewer, &GisViewer::mapClicked, viewer, [&](
        ...
        showAttributes(*detailsTable, hit);
        window.statusBar()->showMessage(QStringLiteral("attributes() returned %1 field(s) for %2 feature %3")
            .arg(hit.attributes().size())
